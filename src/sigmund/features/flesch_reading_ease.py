...
class FleschExtractor(Component):
    ...
    def __init__(self):
        super().__init__(
            name=FleschExtractor.__name__, required_extensions=[],
            creates_extensions=[FRE_DOCUMENT_M, FRE_DOCUMENT_F, FRE_DOCUMENT_MF,
                                FRE_PARAGRAPH_M, FRE_PARAGRAPH_F, FRE_SENTENCE_M,
                                FRE_SENTENCE_F])
    ...
    def apply(self, storage: Dict[Extension, pd.DataFrame],
        ...
        # Filter columns
        fre_sentence = fre_sentence[['document_id', 'couple_id',
                                     'paragraph_id', 'sentence_id', 'gender', 'text']]
        fre_paragraph = fre_paragraph[['document_id',
                                       'couple_id', 'paragraph_id', 'gender', 'text']]
        fre_document = fre_paragraph[['document_id',
                                      'couple_id', 'paragraph_id', 'gender', 'text']]
        ...
        fre_sentence['text'] = fre_sentence['text'].apply(
            self._reading_ease_german, nlp=queryable.nlp())
        fre_paragraph['text'] = fre_paragraph['text'].apply(
            self._reading_ease_german, nlp=queryable.nlp())

        fre_document_M_F = fre_document.groupby(
            ['document_id', 'gender'])['text'].apply(
            self._process_fre, nlp=queryable.nlp())
        fre_document_MF = fre_document.groupby(
            ['document_id'])['text'].apply(
            self._process_fre, nlp=queryable.nlp())
        ...
        fre_document_m = fre_document.drop(
            columns=['fre_document_f', 'fre_document_mf'])
        fre_document_f = fre_document.drop(
            columns=['fre_document_m', 'fre_document_mf'])
        fre_document_mf = fre_document.drop(
            columns=['fre_document_m', 'fre_document_f'])

        # Aggregate over sentences and paragraphs to get document features and drop gender

        fre_sentence_m = fre_sentence_m.drop(
            columns=['gender', 'paragraph_id', 'sentence_id'])
        fre_sentence_f = fre_sentence_f.drop(
            columns=['gender', 'paragraph_id', 'sentence_id'])
        ...
        fre_sentence_m = fre_sentence_m.groupby(
            ['document_id', 'couple_id']).agg('mean').reset_index()
        fre_sentence_f = fre_sentence_f.groupby(
            ['document_id', 'couple_id']).agg('mean').reset_index()
        fre_paragraph_m = fre_paragraph_m.groupby(
            ['document_id', 'couple_id']).agg('mean').reset_index()
        fre_paragraph_f = fre_paragraph_f.groupby(
            ['document_id', 'couple_id']).agg('mean').reset_index()
