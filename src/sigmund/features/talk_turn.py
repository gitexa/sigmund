...
class TalkTurnExtractor(Component):
    ...
    def apply(self, storage: Dict[Extension, pd.DataFrame],
        ...
        tokens_par['tokens_paragraph'] = tokens_par['tokens_paragraph'].apply(
            lambda x: 1 if x > 5 else 0)
        tokens_par = tokens_par.groupby(['document_id', 'speaker'])[
            'tokens_paragraph'].sum()
        ...
        talkturns = pd.DataFrame(values,
                                 columns=['couple_id', 'TalkTurn'])

        #alkturns['is_depressed_group'] = talkturns['is_depressed_group'].astype(bool)
