...
class FleschExtractor(Component):
    ...
    def apply(self, storage: Dict[Extension, pd.DataFrame],
        ...
        fre_sentence = fre_sentence[['document_id', 'couple_id',
                                     'paragraph_id', 'sentence_id', 'speaker', 'text']]
        fre_paragraph = fre_paragraph[['document_id',
                                       'couple_id', 'paragraph_id', 'speaker', 'text']]
        ...
        fre_document_A_B = fre_document.groupby(['document_id', 'speaker'])[
            'text'].apply(process_fre)
        fre_document_AB = fre_document.groupby(
        ...
        values = np.concatenate(
            (np.arange(0, doc_count + 1, dtype=int),
             np.array(couple_ids),
             fre_document_A_B[0:: 2],
             fre_document_A_B[1:: 2],
             fre_document_AB),
            axis=0).reshape(
            (5, 10)).transpose()

        fre_document = pd.DataFrame(
            values, columns=['document_id', 'couple_id', 'FRE_A', 'FRE_B', 'FRE_AB'])
